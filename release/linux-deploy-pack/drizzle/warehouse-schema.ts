...
export const warehouses = pgTable('warehouses', {
  ...
  // UWB支持
  uwbEnabled: boolean().default(false),
  uwbGatewayIp: varchar({ length: 50 }),
  // 状态
...
export const warehouseLocations = pgTable('warehouse_locations', {
  ...
  // UWB实时定位
  uwbAnchorId: varchar({ length: 50 }),
  uwbCoordX: decimal({ precision: 10, scale: 4 }),
  uwbCoordY: decimal({ precision: 10, scale: 4 }),
  uwbCoordZ: decimal({ precision: 10, scale: 4 }),
  uwbLastSeen: timestamp({ mode: 'string' }),
  // 时间戳
  createdAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
}, (table) => [
