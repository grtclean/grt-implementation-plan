...
SELECT 
  ...
  jc.created_at,
  NOW() AS updated_at
FROM jdy_customers jc
LEFT JOIN industry_mapping ind ON jc.industry = ind.jdy_industry
...
SELECT 'customers' AS table_name,
  (SELECT COUNT(*) FROM jdy_customers WHERE is_deleted = 0) AS source_count,
  (SELECT COUNT(*) FROM crm_customers) AS target_count,
  CASE WHEN (SELECT COUNT(*) FROM jdy_customers WHERE is_deleted = 0) = 
            (SELECT COUNT(*) FROM crm_customers) 
       THEN 'PASS' ELSE 'FAIL' END AS status;

SELECT 'opportunities' AS table_name,
  (SELECT COUNT(*) FROM jdy_opportunities WHERE is_deleted = 0) AS source_count,
  (SELECT COUNT(*) FROM crm_opportunities) AS target_count,
  CASE WHEN (SELECT COUNT(*) FROM jdy_opportunities WHERE is_deleted = 0) = 
...
SELECT 'total_contract_amount' AS check_name,
  (SELECT SUM(contract_amount) FROM jdy_contracts WHERE is_deleted = 0) AS source_sum,
  (SELECT SUM(total_amount) FROM contracts) AS target_sum,
  CASE WHEN ABS((SELECT SUM(contract_amount) FROM jdy_contracts WHERE is_deleted = 0) - 
