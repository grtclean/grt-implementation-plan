...
export const helpContentMap: Record<string, PageHelpContent> = {
  ...
  "/meeting-executive": {
    ...
    overview: "G-IME (GRT Intelligent Meeting Executive) 是一个AI驱动的会议效能分析平台。通过分析会议内容块（决策、行动项、发言）自动评估每位参会者的贡献度，并计算会议整体效能评分。分析结果自动关联到员工绩效追踪系统，形成「会议贡献→绩效指标」的完整链路。Phase 2 新增部门汇总分析、会议模式检测、HR智能信号和实时会议助手四大功能模块。",
    steps: [
      ...
      { title: "部门分析", description: "在「部门分析」标签选择部门和时段，查看部门会议汇总、跨部门对比图表和排名" },
      { title: "会议模式检测", description: "在「会议模式」标签点击「检测模式」，自动发现时间浪费、持续低效、最佳模式等会议模式，并查看会议文化报告" },
      { title: "HR信号生成", description: "在「HR信号」标签输入员工ID生成AI信号（晋升就绪、需要培训、参与度下降、领导力涌现），支持确认/忽略操作" },
      { title: "实时会议助手", description: "在「实时助手」标签输入会议ID开启实时会话，通过WebSocket发送转录文本，实时查看贡献分布和每10段自动生成的AI建议" },
    ],
    faq: [
      ...
      { question: "部门分析如何工作？", answer: "通过关联meeting_contributions与hrm_employees表，按部门聚合会议数、平均效能、参与者数等指标，支持跨部门对比和管理看板。" },
      { question: "会议模式检测能发现什么？", answer: "可检测四种模式：时间浪费（效能<40的重复会议）、持续低效（效能下降趋势）、会议过多、最佳模式（效能>80）。同时生成会议文化健康报告。" },
      { question: "HR信号的置信度如何理解？", answer: "置信度0-1表示AI对该信号的确定程度。建议关注置信度≥0.6的信号。信号类型包括晋升就绪、需要培训、参与度下降和领导力涌现。" },
      { question: "实时助手需要什么条件？", answer: "需要一个有效的会议ID。开启实时会话后通过WebSocket连接，输入发言人和转录文本即可。每处理10个语音段会自动生成AI建议。结束会话后自动触发完整的贡献分析和效能评分。" },
    ],
  },
};
