...
import { ExpenseComparisonSkeleton } from "@/components/PageSkeleton";
import { 
  TrendingUp, 
  TrendingDown, 
  Minus,
...
type ComparisonType = 'month_over_month' | 'year_over_year' | 'quarter_over_quarter' | 'custom';
...
export default function ExpenseComparison() {
  ...
  const exportMutation = (trpc.expenseComparisonExport as any).exportReport.useMutation({
    onSuccess: (data) => {
      ...
      toast.success('导出成功', {
        description: `文件已下载: ${data.filename}`,
      });
  ...
  const handleExport = async () => {
    ...
    try {
      // 映射dimension值
      const dimensionMap: Record<string, 'total' | 'department' | 'employee' | 'category' | 'destination'> = {
        'overall': 'total',
        ...
        <Tabs defaultValue="comparison" className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Card className="bg-card/50 border-border">
                    <CardContent className="p-6">
                      ...
                      <div className="text-2xl font-bold font-heading">
                        ¥{comparisonData.currentPeriod?.totalExpense?.toLocaleString() || 0}
                      </div>
                  ...
                  <Card className="bg-card/50 border-border">
                    <CardContent className="p-6">
                      ...
                      <div className="text-2xl font-bold font-heading">
                        ¥{comparisonData.previousPeriod?.totalExpense?.toLocaleString() || 0}
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">
                  ...
                  <Card className="bg-card/50 border-border">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge variant={comparisonData.changeRate > 0 ? 'destructive' : 'default'}>
                      ...
                      <div className={`text-2xl font-bold font-heading ${getChangeColor(comparisonData.changeRate)}`}>
                        {comparisonData.changeAmount > 0 ? '+' : ''}¥{comparisonData.changeAmount?.toLocaleString() || 0}
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">
                        较上期{comparisonData.changeAmount > 0 ? '增加' : '减少'}
                      </div>
                    </CardContent>
                  </Card>
                </div>
                          ...
                          <tbody>
                            {comparisonData.breakdown.map((item: any, index: number) => (
                              <tr key={index} className="border-b border-border/50 hover:bg-muted/20">
                                ...
                                <td className="text-right py-3 px-4">¥{item.previousValue?.toLocaleString()}</td>
                                <td className={`text-right py-3 px-4 ${getChangeColor(item.changeRate)}`}>
                                  {item.changeAmount > 0 ? '+' : ''}¥{item.changeAmount?.toLocaleString()}
                                </td>
                                <td className={`text-right py-3 px-4 ${getChangeColor(item.changeRate)}`}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      {trendData.map((item: any, index: number) => {
                        const maxExpense = Math.max(...trendData.map((d: any) => d.totalExpense || 0));
                        const percentage = maxExpense > 0 ? ((item.totalExpense || 0) / maxExpense) * 100 : 0;
                        return (
          ...
          <TabsContent value="quarter" className="space-y-4">
            <Card className="bg-card/50 border-border">
              ...
              <CardContent>
                {quarterData && quarterData.length > 0 ? (
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  >
                    <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
                      ...
                      <path d="M14 2v6h6"/>
                      <path d="M8 13h2l2 3 2-3h2" stroke="currentColor" strokeWidth="1.5" fill="none"/>
                    </svg>
                    <span className="text-sm font-medium">Excel</span>
                    <span className="text-xs text-muted-foreground">多工作表</span>
                  </Button>
                  <Button
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground">专业报表</span>
                  </Button>
                  <Button
                  ...
                  >
                    <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6z"/>
                      <path d="M14 2v6h6"/>
                      <text x="6" y="17" fontSize="5" fontWeight="bold" fill="currentColor">TXT</text>
                    </svg>
              ...
              <div className="p-3 bg-muted/50 rounded-lg text-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  <Badge variant="outline">
                    {comparisonTypes.find(t => t.value === comparisonType)?.label || comparisonType}
                  </Badge>
                  <Badge variant="outline">
