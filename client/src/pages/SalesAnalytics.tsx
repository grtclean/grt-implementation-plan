...
export default function SalesAnalytics() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        icon={BarChart3}
        title={t("crm.sales.analyticsTitle")}
        description={`${t("crm.sales.analyticsDesc")} · ${t("crm.sales.dataScope")}: ${dataScope}`}
        actions={currentBU ? <Badge variant="outline"><Building2 className="h-3 w-3 mr-1" />{currentBU}</Badge> : undefined}
      />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={DollarSign} label={t("crm.sales.totalRevenue")} value="¥65.2M" iconColor="text-primary" iconBg="bg-primary/10" />
        <StatCard icon={Target} label={t("crm.sales.targetAchievement")} value="86%" iconColor="text-green-500" iconBg="bg-green-500/10" />
        <StatCard icon={TrendingUp} label={t("crm.sales.closedDeals")} value={63} />
        <StatCard icon={BarChart3} label={t("crm.sales.pipeline")} value="¥99.5M" iconColor="text-blue-500" iconBg="bg-blue-500/10" />
      </div>

      <Card>
        <CardHeader><CardTitle>{t("crm.sales.buComparison")}</CardTitle></CardHeader>
        <CardContent>
          <div className="space-y-4">
            {filtered.map((s, i) => (
              <div key={i} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-muted-foreground">{t("crm.sales.revenue")}: {s.revenue} / {t("crm.sales.target")}: {s.target} · {s.deals}{t("crm.sales.dealsUnit")} · {t("crm.sales.pipeline")}: {s.pipeline}</span>
                </div>
            ...
            {filtered.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                ...
                <p className="font-medium">{t("crm.sales.noData")}</p>
              </div>
            )}
          </div>
        </CardContent>
