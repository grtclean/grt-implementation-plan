...
const assistantGroups: AssistantGroup[] = [
  ...
  },
  {
    ...
    entries: [
      { nameKey: "ai.hubPage.modelMonitor", descKey: "ai.hubPage.modelMonitorDesc", path: "/model-performance-monitor", icon: Activity, color: "bg-orange-500/10 text-orange-500" },
      { nameKey: "ai.hubPage.knowledgeGraph", descKey: "ai.hubPage.knowledgeGraphDesc", path: "/knowledge-graph-approval", icon: Network, color: "bg-indigo-500/10 text-indigo-500" },
      { nameKey: "ai.hubPage.modelTraining", descKey: "ai.hubPage.modelTrainingDesc", path: "/model-training-scheduler", icon: Timer, color: "bg-pink-500/10 text-pink-500" },
      { nameKey: "ai.hubPage.agentManagement", descKey: "ai.hubPage.agentManagementDesc", path: "/agent-unit-management", icon: Cpu, color: "bg-teal-500/10 text-teal-500" },
    ],
  },
];
...
function AssistantCard({ entry, t }: { entry: AssistantEntry; t: (key: string) => string }) {
          ...
          <section key={group.titleKey}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {group.entries.map((entry) => (
                <AssistantCard key={entry.path} entry={entry} t={t} />
              ))}
            </div>
          </section>
        ))}
