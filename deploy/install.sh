...
# drizzle-kit is included in the container image; run push with --force to skip confirmation
${COMPOSE_CMD} run --rm grt-app sh -c "npx drizzle-kit push --force" 2>/dev/null || {
    ...
    log_warn "You can retry manually: ${COMPOSE_CMD} run --rm grt-app sh -c 'npx drizzle-kit push --force'"
}
