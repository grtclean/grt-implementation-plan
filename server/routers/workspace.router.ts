...
export const workspaceRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const countResult = await db!.execute(sql`
        ...
        WHERE ${whereClause}
      `);
      const total = ((countResult as any)[0] as { total: number }[])[0]?.total || 0;
  ...
  get: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const workspace = ((result as any)[0] as Record<string, unknown>[])[0];
      if (!workspace) {
      ...
      if (workspace.owner_id !== userId && ((memberCheck as any)[0] as Record<string, unknown>[]).length === 0 && workspace.visibility === 'private') {
  ...
  create: requirePermission('workspace:preferences:manage')
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const workspaceId = ((idResult as any)[0] as { id: number }[])[0]?.id;
  ...
  update: requirePermission('workspace:preferences:manage')
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (((workspace as any)[0] as Record<string, unknown>[]).length === 0) {
        throw new TRPCError({ code: 'NOT_FOUND', message: '工作区不存在' });
      }
      ...
      const isOwner = ((workspace as any)[0] as { owner_id: number }[])[0]?.owner_id === userId;
      const isAdmin = ((memberRole as any)[0] as { role: string }[])[0]?.role === 'admin';
  ...
  delete: requirePermission('workspace:preferences:manage')
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (((workspace as any)[0] as { owner_id: number }[])[0]?.owner_id !== userId) {
  ...
  addMember: requirePermission('workspace:preferences:manage')
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const role = ((memberRole as any)[0] as { role: string }[])[0]?.role;
  ...
  removeMember: requirePermission('workspace:preferences:manage')
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (((targetRole as any)[0] as { role: string }[])[0]?.role === 'owner') {
  ...
  createDocument: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const idResult = await db!.execute(sql`SELECT LAST_INSERT_ID() as id LIMIT 1000`);
      return { id: ((idResult as any)[0] as { id: number }[])[0]?.id, success: true };
    }),
  ...
  getMessages: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return (((result as any)[0] as Record<string, unknown>[]) || []).reverse();
    }),
  ...
  getStats: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return ((stats as any)[0] as { member_count: number; document_count: number; total_tasks: number; completed_tasks: number; pending_tasks: number; message_count: number }[])[0] || {};
    }),
  ...
  searchUsers: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return ((result as any)[0] as { id: number; name: string; email: string; avatar_url: string }[]) || [];
    }),
  ...
  inviteMember: requirePermission('workspace:preferences:manage')
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existingUser = ((userResult as any)[0] as { id: number; name: string; email: string }[])[0];
