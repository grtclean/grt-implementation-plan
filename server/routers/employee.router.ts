...
export const employeeRouter = router({
  ...
  updateRole: requirePermission('hr:employees:edit')
    .input(z.object({
      employeeId: z.string(),
  ...
  batchUpdateRoles: requirePermission('hr:employees:edit')
    .input(z.object({
  ...
  updateStatus: requirePermission('hr:employees:edit')
    .input(z.object({
      employeeId: z.string(),
  ...
  // 初始化员工数据（从JSON导入）
  initFromData: requirePermission('hr:employees:edit').mutation(async () => {
    // 从图片解析的员工数据
    const employees = [
      ...
      {employeeId: "GRT055", name: "沈迎凤", department: "事业三部", position: "商务经理"},
      ...
      {employeeId: "GRT063", name: "刘健康", department: "事业一部", position: "销售经理"},
  ...
  // 从外部数据平台同步员工数据
  syncFromExternalSync: requirePermission('hr:employees:edit').mutation(async () => {
    // TODO: 实际从外部数据平台API获取数据
    // 这里返回模拟结果
    return {
      added: 0,
      updated: 0,
