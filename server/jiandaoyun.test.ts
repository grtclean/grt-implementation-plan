...
import { getJiandaoyunSyncService, resetJiandaoyunSyncService, JiandaoyunClient } from "./jiandaoyun";

describe("Jiandaoyun API", () => {
  beforeEach(() => {
    // Reset singleton before each test
    resetJiandaoyunSyncService();
  });
  ...
  it("should check if API key is configured", () => {
    ...
    console.log(`Jiandaoyun API configured: ${isConfigured}`);
  ...
  it("should check if corp ID is configured", () => {
    ...
    console.log(`Jiandaoyun Corp ID configured: ${corpId ? "Yes (" + corpId + ")" : "No"}`);
  ...
  it("should return stats (mock or real)", () => {
    ...
    console.log("Jiandaoyun stats:", stats);
  });
  ...
  it("should test API connection", { timeout: 30000 }, async () => {
    ...
    if (result.success) {
      ...
      if (result.message.includes("401") || result.message.includes("Unauthorized")) {
        console.error("❌ Invalid API credentials - please check JIANDAOYUN_API_KEY and JIANDAOYUN_CORP_ID");
      }
    }
  });
  ...
  it("should get forms for an app when API is configured", { timeout: 15000 }, async () => {
    ...
    try {
      ...
      console.warn('⚠️ Get forms failed:', error.message);
    }
  });
});
