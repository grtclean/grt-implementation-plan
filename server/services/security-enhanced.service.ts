...
export class AlertWebhookService {
  ...
  static async getConfigs(): Promise<WebhookConfig[]> {
    ...
    return (rows as any[]).map((row: any) => ({
      id: row.id, name: row.name, channelType: row.channel_type,
      webhookUrl: row.webhook_url, secret: row.secret, enabled: row.enabled,
      alertTypes: JSON.parse(row.alert_types || "[]"), minSeverity: row.min_severity,
      rateLimit: row.rate_limit, template: row.template,
    }));
  }
  ...
  static async sendAlert(alert: SecurityAlert): Promise<SendResult[]> {
    ...
    for (const config of rows as any[]) {
      ...
      try {
        ...
        await this.sendToWebhook(config.channel_type, config.webhook_url, message, config.secret);
        await db.execute(
        ...
        await db.execute(
          ...
          [config.id, alert.id, false, error.message]
        );
        results.push({ webhookId: config.id, webhookName: config.name, success: false, error: error.message });
      }
    }
    return results;
  }

  private static buildMessage(alert: SecurityAlert, config: any): any {
  ...
  private static async sendToWebhook(channelType: string, url: string, message: any, secret?: string): Promise<any> {
...
export class LogArchiveService {
  ...
  static async executeArchive(policyId: number): Promise<ArchiveResult> {
    ...
    if ((policyRows as any[]).length === 0) throw new Error("策略不存在");
  ...
  static async getArchiveHistory(policyId?: number, limit: number = 20): Promise<any[]> {
    const db = await requireDb() as any;
  ...
  static async getStorageStats(): Promise<any> {
    ...
    for (const table of tables) {
      try {
        ...
        stats[table.type] = { count: (countRows as any[])[0].count, oldestDate: (countRows as any[])[0].oldest };
    ...
    return { logStats: stats, totalArchived: (archiveRows as any[])[0].total || 0 };
  }
}
