...
const log = createChildLogger("ext-sync-import");
 ...
 * 生成唯一的实例/模板编码
 */
function generateCode(prefix: string, extId: string): string {
  return `${prefix}-EXT-${extId.substring(0, 12)}`;
}
...
interface ParsedWorkflowStep {
  ...
  approverRole: string;
  approverType: string;
  approverRef?: unknown; // 外部平台用户引用，运行时解析
}

/**
 * 审批流程导入服务
 */
export class ExtSyncApprovalImportService {
   ...
   * 兜底: 返回默认单步骤
   */
  private parseWorkflowSteps(record: ExternalSyncRecord): ParsedWorkflowStep[] {
      ...
      try {
        ...
        for (const record of allRecords) {
          try {
            ...
            result.failed++;
            result.errors.push({ entity: 'approval_template', extId: record._id, message: error.message });
          }
        }
      } catch (error: any) {
        result.errors.push({ entity: 'approval_template', extId: mapping.jdy_form_id, message: error.message });
      }
    }
      ...
      try {
        ...
        for (const record of allRecords) {
          try {
            ...
            result.failed++;
            result.errors.push({ entity: 'approval_instance', extId: record._id, message: error.message });

            // 检查失败率
            const total = result.instancesCreated + result.instancesUpdated + result.failed;
            if (total > 10 && result.failed / total > 0.5) {
              result.errors.push({ entity: 'approval_instance', extId: '', message: '失败率超过50%，导入已暂停' });
              return result;
            }
          }
        }
      } catch (error: any) {
        result.errors.push({ entity: 'approval_instance', extId: mapping.jdy_form_id, message: error.message });
      }
    }
...
// 单例
let instance: ExtSyncApprovalImportService | null = null;
export function getExtSyncApprovalImportService(): ExtSyncApprovalImportService {
  if (!instance) {
    instance = new ExtSyncApprovalImportService();
  }
  return instance;
}
