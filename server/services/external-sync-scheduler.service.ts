...
export interface SyncTaskConfig {
  ...
  extAppId?: string;
  extFormId?: string;
  syncDirection: 'ext_to_grt' | 'grt_to_ext' | 'bidirectional';
...
export class ExternalSyncScheduler {
  private syncService = getExternalSyncService();
  private userSyncService = getExternalSyncUserService();
    ...
    const result = await db.execute(
      drizzleSql`INSERT INTO jiandaoyun_sync_tasks 
        ...
        VALUES (
          ${config.taskName}, 
          ${config.taskType}, 
          ${config.extAppId || null}, 
          ${config.extFormId || null}, 
...
// 单例实例
let schedulerInstance: ExternalSyncScheduler | null = null;

export function getExternalSyncScheduler(): ExternalSyncScheduler {
  if (!schedulerInstance) {
    schedulerInstance = new ExternalSyncScheduler();
  }
