/**
 * 外部平台表单发现服务
 * 自动发现外部平台应用/表单，分类为GRT目标实体，生成字段映射建议
 */

import { requireDb } from '../db';
import { sql as drizzleSql } from 'drizzle-orm';
import { getExternalSyncService, ExternalSyncField } from '../external-sync';
import { createChildLogger } from '../lib/logger';

const log = createChildLogger("ext-sync-forms");
...
// GRT字段映射建议规则（外部平台字段label → GRT column）
 ...
 * 表单发现服务
 */
export class ExtSyncFormDiscoveryService {
  /**
   * 执行表单发现 - 扫描所有外部平台应用和表单，自动分类并存储映射
   */
  async discoverForms(): Promise<FormDiscoveryResult> {
    const syncService = getExternalSyncService();
    if (!syncService.isConfigured()) {
      throw new Error('外部平台API未配置');
    }
    ...
    // Circuit breakers — stop calling APIs that consistently 403
    let fieldsApiAvailable = true;
    let statsApiAvailable = true;
    let dataApiAvailable = true;
    ...
    for (const app of apps) {
      try {
        ...
        for (const form of forms) {
          ...
          try {
            // 3. 获取字段结构 (circuit breaker: skip if first call 403'd)
            let fields: ExternalSyncField[] = [];
            if (fieldsApiAvailable) {
              try {
                fields = await syncService.getFormFields(app.app_id, form.entry_id);
              } catch {
                fieldsApiAvailable = false;
                log.info("getFormFields 403 — disabling for remaining forms (API key lacks scope)");
              }
            }

            // 4. 获取记录数 (circuit breaker: skip if first call 403'd)
            let recordCount = 0;
            if (statsApiAvailable) {
              try {
                ...
                log.info("getFormStats 403 — disabling for remaining forms (API key lacks scope)");
              }
            }
            ...
            // 6. 生成字段映射建议
            const fieldMapping = targetEntity && fields.length > 0
              ? this.suggestFieldMapping(fields, targetEntity)
              : null;

            // 7. 获取样例数据 — only if circuit breaker open AND we have records
            let sampleData: unknown[] = [];
            if (dataApiAvailable && recordCount > 0) {
              try {
                ...
                log.info("getFormData 403 — disabling for remaining forms (API key lacks scope)");
              }
            }
            ...
            const existingRows = (existing as any).rows || (existing as any)[0] || [];
            ...
            if (existingRows.length > 0) {
              await db.execute(
                drizzleSql`UPDATE jiandaoyun_form_mappings SET
                  ...
                  field_schema = ${fields.length > 0 ? JSON.stringify(fields) : null},
              ...
              await db.execute(
                ...
                VALUES (${app.app_id}, ${app.name}, ${form.entry_id}, ${form.name}, ${targetEntity},
                  ...
                  ${fields.length > 0 ? JSON.stringify(fields) : null}, false, NOW())`
              );
            }
  ...
  /**
   * 从外部API拉取表单数据并缓存到本地 jiandaoyun_form_data_cache 表
   * 仅在 EXT_SYNC_API_KEY 配置时可用（管理员手动触发）
   */
  async cacheFormData(appId: string, formId: string): Promise<{ cached: number; errors: number }> {
    const service = getExternalSyncService();
    if (!service.isConfigured()) {
      throw new Error("外部平台API未配置，无法拉取数据");
    }
    const client = (service as any).client || service;
    const allData = await (client.getAllFormData ? client.getAllFormData(appId, formId) : service.getAllFormData(appId, formId));
    const db = await requireDb();
    ...
    for (const record of allData) {
      try {
        ...
        await db.execute(
          drizzleSql`INSERT INTO jiandaoyun_form_data_cache
            (jdy_app_id, jdy_form_id, jdy_record_id, record_data, ext_creator, ext_created_at, ext_updated_at, synced_at)
          VALUES (${appId}, ${formId}, ${record._id || String(cached)}, ${recordJson}::json,
                  ${record.creator || null}, ${record.createTime || null}, ${record.updateTime || null}, NOW())
          ON CONFLICT ON CONSTRAINT uk_form_data_cache_record
          DO UPDATE SET record_data = ${recordJson}::json, synced_at = NOW()`
        );
        cached++;
      } catch (err) {
        log.warn({ err, appId, formId }, "Failed to cache form data record");
        errors++;
      }
    }

    log.info({ appId, formId, cached, errors }, "Form data cached to local DB");
    return { cached, errors };
  }
}

// 单例
let instance: ExtSyncFormDiscoveryService | null = null;
export function getExtSyncFormDiscoveryService(): ExtSyncFormDiscoveryService {
  if (!instance) {
    instance = new ExtSyncFormDiscoveryService();
  }
  return instance;
}
