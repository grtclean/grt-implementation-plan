 ...
 * 项目数据导入服务
 */
export class JdyProjectImportService {
  ...
  /**
   * 解析JDY用户引用为GRT userId
   * JDY中用户引用可能是 username 字符串 或 { username, name } 对象
   */
  ...
  private mapRecordToProject(
    ...
    // 设置JDY ID用于幂等
    mapped.jiandaoyunId = record._id;
...
// 单例
let instance: JdyProjectImportService | null = null;
export function getJdyProjectImportService(): JdyProjectImportService {
  if (!instance) {
    instance = new JdyProjectImportService();
  }
  return instance;
}
