/**
 * 简道云知识库导入服务
 * 从 jiandaoyun_form_mappings 中挖掘元数据生成知识文档
 ...
 * 知识库导入服务
 */
export class JdyKnowledgeImportService {
  /**
   * 从表单元数据生成知识文档
   */
    ...
    for (const mapping of mappings) {
      try {
        ...
        if (Object.keys(fieldMapping).length > 0) {
          ...
          contentParts.push('');
          contentParts.push('| JDY字段 | GRT列 |');
          contentParts.push('|---------|-------|');
          for (const [jdyField, grtCol] of Object.entries(fieldMapping)) {
            contentParts.push(`| ${jdyField} | ${grtCol} |`);
          }
          contentParts.push('');
        }
        ...
        // 用标题去重 (source = 'jdy_import')
        const existing = await db.execute(
          drizzleSql`SELECT id FROM knowledge_documents
            WHERE source = 'jdy_import' AND title = ${title} LIMIT 1`
        );
...
// 单例
let instance: JdyKnowledgeImportService | null = null;
export function getJdyKnowledgeImportService(): JdyKnowledgeImportService {
  if (!instance) {
    instance = new JdyKnowledgeImportService();
  }
  return instance;
}
