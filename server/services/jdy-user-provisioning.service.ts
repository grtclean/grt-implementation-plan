/**
 * 简道云用户开通服务
 * 将 jiandaoyun_user_mappings 中的成员自动开通为 GRT 系统用户
 */
...
/**
 * 查询 jiandaoyun_dept_mappings 获取部门中文名
 */
...
export interface ProvisionResult {
  ...
  errors: Array<{ jdyUsername: string; error: string }>;
  details: Array<{
    jdyUsername: string;
    jdyName: string;
...
/**
 * 核心：从 jiandaoyun_user_mappings 批量开通用户
 * @param dryRun - true 时仅预览，不写入数据库
 */
export async function provisionUsersFromJiandaoyun(dryRun: boolean = false): Promise<ProvisionResult> {
  ...
  const mappingsResult = await db.execute(sql`
    SELECT id, jdy_username, jdy_name, jdy_departments, jdy_status,
           grt_user_id, grt_open_id, sync_status
  ...
  for (const mapping of mappings) {
    try {
      const jdyUsername = mapping.jdy_username;
      const jdyName = mapping.jdy_name;
      const jdyDepts = mapping.jdy_departments as number[] | null;
      const grtUserId = mapping.grt_user_id;

      // 解析部门
      const departmentName = await resolveDepartmentName(db, jdyDepts);
      const buCode = DEPARTMENT_TO_BU[departmentName] || null;

      if (!grtUserId) {
        // 场景 1 & 2：grt_user_id 为空
        // 检查 users 表是否已有该 openId
        const existingUserResult = await db.execute(sql`
          SELECT id, name FROM users WHERE "openId" = ${jdyUsername} LIMIT 1
        `);
        ...
        if (existingUsers.length > 0) {
          ...
          if (!dryRun) {
            await db.execute(sql`
              ...
              SET grt_user_id = ${userId}, grt_open_id = ${jdyUsername},
          ...
          // 场景 1：创建新用户
          // 查找/生成员工编号
          const empInfo = await getOrGenerateEmployeeId(db, jdyName);
          ...
          if (!dryRun) {
            ...
            const insertResult = await db.execute(sql`
              ...
              VALUES (${jdyUsername}, ${jdyName}, ${loginMethod}, ${userRole}, NOW(), NOW(), NOW())
              RETURNING id
            `);
            ...
            await db.execute(sql`
              UPDATE jiandaoyun_user_mappings
              SET grt_user_id = ${newUserId}, grt_open_id = ${jdyUsername},
                  sync_status = 'synced', last_sync_at = NOW(), updated_at = NOW()
              WHERE id = ${mapping.id}
            `);
            ...
            if (empInfo.isExisting) {
              ...
              await db.execute(sql`
                ...
                VALUES (${empInfo.employeeId}, ${jdyName}, ${departmentName}, ${"员工"}, ${buCode}, 'active')
              `);
        ...
        // 检查姓名是否变化
        const nameChanged = existingUser.name !== jdyName;
        if (nameChanged && !dryRun) {
          await db.execute(sql`
            UPDATE users SET name = ${jdyName}, "updatedAt" = NOW()
            WHERE id = ${grtUserId}
          `);
        }
        ...
        const empResult = await db.execute(sql`
          ...
          WHERE name = ${jdyName} AND status = 'active'
          LIMIT 1
        `);
        ...
        if (nameChanged || deptChanged) {
          ...
          result.details.push({
            ...
            department: departmentName,
            message: [
              nameChanged ? `姓名更新: ${existingUser.name} → ${jdyName}` : null,
      ...
      result.errors.push({
        jdyUsername: mapping.jdy_username,
        error: error.message,
      });
      result.details.push({
        jdyUsername: mapping.jdy_username,
        jdyName: mapping.jdy_name,
        action: "error",
