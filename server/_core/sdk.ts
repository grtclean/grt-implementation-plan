...
// Storage key for session token (must match client-side)
...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    ...
    if (!ENV.oAuthServerUrl) {
      log.error("OAUTH_SERVER_URL not configured — set OAUTH_SERVER_URL environment variable");
    }
  }
...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // Fallback: check Authorization header for Bearer token (from localStorage)
    if (!sessionCookie) {
