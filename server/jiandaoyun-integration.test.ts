...
vi.mock('./_core/env', () => ({
  ENV: {
    JIANDAOYUN_API_KEY: 'test-api-key',
    JIANDAOYUN_CORP_ID: 'test-corp-id',
  },
}));

describe('简道云API集成 v1.3.24', () => {
  describe('API配置验证', () => {
    it('应该正确配置API密钥', () => {
      const apiKey = process.env.JIANDAOYUN_API_KEY || 'test-api-key';
      expect(apiKey).toBeDefined();
      expect(apiKey.length).toBeGreaterThan(0);
    });

    it('应该正确配置企业ID', () => {
      const corpId = process.env.JIANDAOYUN_CORP_ID || 'test-corp-id';
...
console.log('v1.3.24 简道云API集成测试完成');
