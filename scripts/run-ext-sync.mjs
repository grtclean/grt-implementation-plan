...
const { getExternalSyncService, getExternalSyncUserService } = await import('../server/external-sync.ts');
...
if (!syncService.isConfigured()) {
  console.error('外部平台 API 未配置，请检查 .env');
  process.exit(1);
}
...
const tables = ['external_sync_user_mappings', 'external_sync_dept_mappings', 'external_sync_role_mappings', 'external_sync_role_members'];
